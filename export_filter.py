        ...
        if export_file_info and messages:
            ...
            if download_url:
                ...
                if download_url not in content:
                    # Add the download link - use HTML to force download behavior
                    # Markdown links open in new tab, so we use HTML with download attribute
                    # Note: The download attribute may not work for cross-origin URLs, but Content-Disposition header will handle it
                    export_note = (
                        f"\n\n---\n"
                        f"{icon} **Your {export_format.upper()} Export is Ready!**\n\n"
                        f"<a href=\"{download_url}\" download=\"{filename}\" style=\"display: inline-block; padding: 8px 16px; background-color: {branding['primary_color']}; color: white; text-decoration: none; border-radius: 4px; font-weight: bold;\">📥 Download {filename}</a>\n\n"
                        f"*File size: {file_size_kb}KB | Generated by {branding['company_name']}*\n"
                        f"*Note: If download doesn't start, right-click the link and select 'Save As'*"
                    )
